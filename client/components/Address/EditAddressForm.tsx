...
import {
  ...
  isValidStreet, isValidSex, isValidTitleInFront, isValidTitleBehind, isValidAddress, concatUnique, prettyShortAddress, filterEditedDuplicate
} from "../../utils/address";
import AddressTable from "../Babybox/AddressTable";
...
import { putAddress } from "../../api/address/putAddress";

interface EditAddressFormProp {
  babyboxHandle: string,
  addressProp: Address,
  close: Function
}

export default function EditAddressForm({ babyboxHandle, addressProp, close }: EditAddressFormProp) {
  ...
  const duplicatesFiltered = filterEditedDuplicate(duplicates, address)
  const submitAddress = async () => {
    ...
    try {
      const result = await putAddress(address)
      triggerDuplicates(babyboxHandle, address.company, address.email)
      triggerDuplicates(babyboxHandle, addressProp.company, addressProp.email)
      close()
      return toast({
        title: "Adresa úspěšně editována.",
        description: `Adresa editována na: ${prettyShortAddress(address)}`,
        status: "success",
        isClosable: true,
      })
    } catch (err) {
      console.log(err)
      return toast({
        title: "Při editování adresy se vyskytla chyba.",
        description: `Adresa nebyla editována - zkontrolujte, jestli není adresa duplicitní.`,
        status: "error",
        isClosable: true,
      })
    }
  ...
  ) : (!duplicatesFiltered || duplicatesFiltered.length === 0) ? (
  ...
  const duplicateContentJSX = duplicateError ? (
    ...
    <AddressTable addresses={duplicatesFiltered || null} handle={babyboxHandle} address={address} editButton={false} />
  ) : (<></>)
  ...
  return (
    <Box>
      ...
      <HStack alignItems="flex-start" mt={2} wrap="wrap">
